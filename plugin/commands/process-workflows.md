...
4. **Before invoking developer-agent for this step**, resolve DataOps platform metadata yourself, in this top-level session: call `list_engines`/`list_folders`/`list_data_sources` (scoped to the target container from `dataflow-environment-config`) directly. This is required because MCP server tools don't propagate to Agent-tool subagents (see `orchestrator-agent.md`'s Known limitation section) — skipping it lets developer-agent try and fail, wasting a full subagent call that can only report the blocker.
   Then invoke `developer-agent` with those resolved values (container/engine/folder names, and real `dataSourceName` values matched against the container's registered sources) embedded directly in the prompt, instructing it to use them as-is rather than attempting its own `list_engines`/`list_folders`/`list_data_sources` calls. It will still ask you what the dataflow should actually do (implement the ETL vs. validate/reconcile) if `dataflow-environment-config` doesn't already answer that — answer per-file, since this command does not default it. Per its own instructions, developer-agent then also runs the dataflow and downloads the DataCompare report to `Results/` automatically.
5. If the run Failed, developer-agent hands off to the `analysis-agent` subagent (a separate `Agent` tool call — never write the analysis yourself or fold this into developer-agent's own output, even when working around the MCP-tools-don't-propagate-to-subagents limitation, since analysis-agent needs no MCP tools at all) to classify each failure (test case defect / environment defect / genuine workflow bug, cross-checked against the workflow XML) and write `Results/<Name>_run<id>_analysis.html`. Relay its Recommended Actions to the user verbatim and wait for explicit confirmation before applying anything — a run that Failed is not "done" until this classify-and-confirm step has actually happened, even if the eventual answer is "this needs a human/SME, nothing more to do here." Never substitute your own guess at root cause for this step.
6. If the user confirms a fix and it's within developer-agent's authority (a test case defect), apply it, re-run, and repeat from step 5. If it's an environment/data defect or genuine workflow/ETL bug, state plainly what needs to change outside this pipeline's scope and stop.
7. Once the run reaches a final state (Passed, or Failed-with-recommendation-delivered), developer-agent commits + pushes the `HRD/`, `Results/`, and `OKF/` files to the current branch's tracked remote — it does not stop after creation. It only pauses again if it hits a genuine blocker (name collision, unresolved parameter, missing prerequisite, or a push conflict).
