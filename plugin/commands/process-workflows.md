...
4. Invoke `developer-agent` again to build the corresponding dataflow on the DataOps platform from that test case. It will ask you which container/project and what the dataflow should actually do (implement the ETL vs. validate/reconcile) — answer per-file, since this command does not default those. Per its own instructions, developer-agent then also runs the dataflow and downloads the DataCompare report to `Results/` automatically.
