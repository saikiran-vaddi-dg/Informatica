...
This compact summary does **not** capture Mapping Variables (`$$` parameters, including unresolved ones) or session-level per-partition SQL overrides — for those, or anything else the summary leaves ambiguous, read the raw workflow XML directly. Don't default to reading the full file when the compact summary already answers the question.
...
From this, draft the test case by invoking the `test-case-generation` skill (via the Skill tool) rather than writing free-text prose — that skill owns the required JSON structure and worked examples; don't restate or reinvent them here.
