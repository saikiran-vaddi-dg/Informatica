...
2. For the workflow(s) in scope, check `OKF/workflows/<WorkflowName>/extraction.md`'s `generated.at` frontmatter first (only `extraction.md` carries this — see Process step 1 below for why), comparing it against `git log -1 --format=%cI -- Workflows/<WorkflowName>.XML` to determine staleness. If it exists and isn't stale, use `extraction.md` (description, key columns) and `hrd_mapping.md` (existing test cases/dataflows/runs) as your summary instead of re-reading the full XML. If it's stale or missing, apply review-agent's Pass 1 staleness/diff-scoped review technique (`agents/review-agent.md`) to amend only what actually changed rather than regenerating the whole concept folder from scratch.
